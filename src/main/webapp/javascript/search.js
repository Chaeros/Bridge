...
function addBookMark(contentId) {
	...
	fetch(serverIp+"/bridge/bookmark?action=add", {
...
function removeBookMark(myAttractionId) {
	...
	fetch(serverIp+"/bridge/bookmark?action=remove", {
		method: 'POST', // 요청 메서드를 POST로 설정합니다.
		headers: {
			'Content-Type': 'application/json' // 요청 헤더에 JSON 형식의 데이터를 전송한다고 명시합니다.
		},
...
function removeBookMarkAndReturnAttractionList(contentId) {
	...
	fetch(serverIp+"/bridge/bookmark?action=removeByContentId", {
		method: 'POST', // 요청 메서드를 POST로 설정합니다.
		headers: {
			'Content-Type': 'application/json' // 요청 헤더에 JSON 형식의 데이터를 전송한다고 명시합니다.
		},
	...
	fetch(serverIp+"/bridge/attractionInfo?action=list", {
		...
		.then((data) => {
			...
			data.forEach((item, index) => {
				...
				let isIn = false;
				fetch(serverIp+"/bridge/bookmark?action=isIn", {
					method: 'POST', // 요청 메서드를 POST로 설정합니다.
					headers: {
						'Content-Type': 'application/json' // 요청 헤더에 JSON 형식의 데이터를 전송한다고 명시합니다.
					},
...
function getOptimalRouteMyBookMarkList(){
	...
	console.log(keyword, areacode, sigunguCode, contentTypeId);
	console.log(serverIp)
	fetch(serverIp+"/bridge/bookmark?action=optimalRoute", {
		method: 'POST', // 요청 메서드를 POST로 설정합니다.
		headers: {
			'Content-Type': 'application/json' // 요청 헤더에 JSON 형식의 데이터를 전송한다고 명시합니다.
		},
...
function makeAttractionDetailActionListener(){
	let count = itemList.length;
	console.log(count);
	for (var i = 0; i < count; ++i) {
		console.log("destination-" + i);
...
function addAttractionBox(item,index){
	let newItem = document.createElement("div");
	newItem.classList.add("destination");
